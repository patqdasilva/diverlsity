...
PolicyLossFn = Callable[
    [
        ...
        torch.Tensor | None,  # response_logits
        torch.Tensor | None,  # action_ids
    ],
...
def _validate_tsallis_stochastic_q2_config(config: DictConfig | ActorConfig | None) -> ActorConfig | DictConfig:
    ...
    ppo_epochs = _get_config_value(config, "ppo_epochs", None)
    if ppo_epochs != 1:
...
def _simplex_project_q2(values: torch.Tensor) -> torch.Tensor:
    ...
    support = sorted_values - cssv / torch.arange(1, values.shape[-1] + 1, device=values.device) > 0
    rho = support.sum(dim=-1).clamp(min=1)
    theta = cssv.gather(dim=-1, index=(rho - 1).unsqueeze(-1)) / rho.unsqueeze(-1)
...
@register_policy_loss(TSALLIS_STOCHASTIC_Q2)
def compute_policy_loss_tsallis_stochastic_q2(
    ...
    del old_log_prob, rollout_log_probs
    ...
    zero = torch.zeros((), device=pg_loss.device)
